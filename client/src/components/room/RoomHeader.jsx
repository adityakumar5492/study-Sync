...
const RoomHeader = ({ room, currentUser, onMenuClick }) => {
    ...
    const hostId = typeof room.host === "object" ? room.host?._id?.toString() : room.host?.toString();
    ...
    const confirmDeleteRoom = async () => {
        ...
        try {
            ...
            toast.error(
                typeof err === "string" ? err : err?.message || "Failed to delete room."
            );
    ...
    const handleLeaveRoom = async () => {
        ...
        if (!room.isPrivate) {
            ...
            try {
                ...
                toast.error(
                    typeof err === "string" ? err : err?.message || "Failed to leave room."
                );
            ...
            >
                                ...
                                <div className="mt-1 flex items-center gap-2.5">
                                    <span className="flex items-center gap-1 text-[8px] font-medium text-zinc-600">
                                        ...
                                        <span>
                                            ...
                                            {room.maxMembers ? `/${room.maxMembers}` : ""} students
                                        </span>
                                    </span>

                                    <span className="h-2.5 w-px bg-white/[0.07]" />

                                    <span className={`flex items-center gap-1 text-[8px] font-semibold ${room.isPrivate ? "text-amber-400/80" : "text-cyan-400/80"}`}>
                                        {room.isPrivate ? (
                                            <>
                            ...
                            {room.isPrivate && isHost && (
                                ...
                                >
                                    ...
                                    <motion.button
                                        ...
                                        className={`flex h-8 items-center gap-1.5 rounded-xl border px-2.5 text-[8px] font-bold transition-all ${copied ? "border-emerald-400/15 bg-emerald-500/[0.08] text-emerald-300" : "border-violet-400/10 bg-violet-500/[0.06] text-violet-300 hover:border-violet-400/20 hover:bg-violet-500/[0.12]"}`}
                                    >
                                        {copied ? <FaCheck className="text-[8px]" /> : <FaCopy className="text-[8px]" />}
                                        {copied ? "Copied" : "Copy"}
                                    </motion.button>
                                </motion.div>
                            )}
                ...
                <div className="relative shrink-0">
                    <motion.button
                        ...
                        whileTap={{ scale: 0.9 }}
                        className={`relative flex h-9 w-9 items-center justify-center rounded-xl border transition-all ${menuOpen ? "border-violet-400/20 bg-violet-500/[0.1] text-white" : "border-white/[0.06] bg-white/[0.025] text-zinc-500 hover:border-white/[0.1] hover:bg-white/[0.05] hover:text-white"}`}
                    ...
                    {showLeaveModal && (
                        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                            ...
                            <motion.div
                                ...
                                className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0c0c12] shadow-[0_30px_100px_rgba(0,0,0,.65)]"
                            >
                                <div className="p-5">
                                    <div className="mb-4 flex items-start gap-3">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="mt-1 text-[10px] leading-relaxed text-zinc-500">
                                                You will leave "{room.name}" and need the invite code to join this private room again.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            type="button"
                                            onClick={() => setShowLeaveModal(false)}
                                            className="flex-1 rounded-xl border border-white/[0.07] bg-white/[0.03] px-4 py-2.5 text-[9px] font-bold text-zinc-400 transition hover:bg-white/[0.06] hover:text-white"
                                        >
                                            Cancel
                                        </button>
                                        ...
                                        <button
                                            ...
                                            disabled={leaving}
                                            className="flex-1 rounded-xl border border-amber-400/10 bg-amber-500/[0.09] px-4 py-2.5 text-[9px] font-bold text-amber-300 transition hover:bg-amber-500/[0.15] disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                            {leaving ? "Leaving..." : "Leave Room"}
                                        </button>
                                    </div>
                    ...
                    {showDeleteModal && (
                        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
                            ...
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: 10, scale: 0.96 }}
                                transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}
                                className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-red-400/[0.08] bg-[#0c0c12] shadow-[0_30px_100px_rgba(0,0,0,.65)]"
                            >
                                <div className="p-5">
                                    <div className="mb-4 flex items-start gap-3">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="mt-1 text-[10px] leading-relaxed text-zinc-500">
                                                Delete "{room.name}" permanently? All members will lose access immediately.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            disabled={deleting}
                                            className="flex-1 rounded-xl border border-red-400/10 bg-red-500/[0.09] px-4 py-2.5 text-[9px] font-bold text-red-300 transition hover:bg-red-500/[0.15] disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                            {deleting ? "Deleting..." : "Delete Room"}
                                        </button>
                                    </div>
